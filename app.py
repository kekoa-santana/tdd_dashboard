...
def main() -> None:
    ...
    for label, children in _NAV:
        if children is None:
            ...
            mobile_nav_html += (
                f'<a href="?page={slug}" target="_self" class="topbar-mob-item{active}"{aria}>{label}</a>'
            )
        else:
            ...
            for child in children:
                ...
                mobile_nav_html += (
                    f'<a href="?page={slug}" target="_self" class="topbar-mob-item{active}"{aria}>{child}</a>'
                )
    ...
    _logo_html = (
        f'<img src="data:image/png;base64,{_logo_b64}" class="topbar-logo" alt="The Data Diamond">' if _logo_b64 else ""
    )
