...
def build_attribution_panel(row: pd.Series | dict, *, key_suffix: str = "") -> str:
    ...
    return (
        ...
        '<div style="color:var(--tdd-gold);font-weight:700;font-size:0.66rem;'
        'letter-spacing:1px;text-transform:uppercase;margin-bottom:0.35rem">'
        f'Why this number &mdash; {escape(stat)} '
        f'<span style="color:var(--tdd-slate);font-weight:400;text-transform:none;'
        f'letter-spacing:0">over ~{volume:.0f} {vol_unit}</span></div>'
        f'{baseline_row}{rows_html}{proj_row}{hidden_note}'
        '</div>'
    )
